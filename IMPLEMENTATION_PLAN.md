...
- **Lane 2 (Link)**: Zero-latency P2P voice link powered by Google Nearby Connections using Wi-Fi Aware/NAN-capable bearers and stream payloads for microphone audio.
...
> - **Audio Transport**: Hold-to-talk audio uses Nearby Connections stream payloads for the lowest-latency microphone path.
...
- Implement Android `ConnectionService` to ensure the OS treats Link as a priority voice call (preventing mic suspension in the pocket).
...
1. **`NearbyConnectionsManager`**: Cross-platform Google Nearby Connections bindings for advertising, discovery, connection acceptance, peer state events, and hold-to-talk stream payload control.
